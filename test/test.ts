...
import app from '../app';
import { User, Candidate, Schedule, Availability, Comment } from '../models';
import { deleteScheduleAggregate } from '../routes/schedules';
...
describe('/login', () => {
    ...
    it('ログイン時はナビゲーションバーにユーザー名が表示される', (done) => {
        supertest(app)
            .get('/')
            .expect(/testuser/)
            .expect(200, done);
